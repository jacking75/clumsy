...
- [x] 4.10 `.rpm` 빌드 검증
- [x] 4.11 히스토리에서 IUP 제거 (58MB → 1.8MB)
- [x] 4.12 Windows 실캡처 회귀 검증
- [x] 4.13 `gh-pages`(upstream 잔여물) 삭제
...
- [x] ~~기존 MinGW(clang) 빌드 동작 확인~~ — **범위에서 제외**(4.9).
      MinGW 빌드 경로는 사용하지 않기로 결정해 `genie.lua`와 함께 제거했습니다.
...
### 4.10 `.rpm` 빌드 검증 — 완료 (2026-08-28)

앞서 "rpm 툴체인 미설치"로 미검증으로 남겼던 항목입니다. WSL에 `rpm` 패키지를 설치하면
Debian에서도 `rpmbuild`(4.18.2)를 쓸 수 있어 실제로 검증했습니다.

- `BuildRequires`가 Fedora 패키지 이름이라 Debian의 rpm DB가 해석하지 못합니다
  (라이브러리 자체는 설치되어 있음). 스펙 검증용으로 `RPM_NODEPS=1` 옵션을 추가했습니다.
  실제 Fedora/RHEL 빌드에서는 의존성 검사가 정상 동작해야 하므로 기본값은 유지합니다.
- **절대 심볼릭 링크 경고 2건을 수정**했습니다. `/usr/bin/config.json`을
  `/usr/share/clumsy/config.json`로 걸면 rpm이 경고할 뿐 아니라, 대체 루트(`--root`)에
  설치했을 때 링크가 호스트를 가리켜 깨집니다. 상대 링크로 바꿨고 `.deb`도 같이 고쳤습니다.

**검증 9항목 통과**: 빌드(경고 0), 의존성 자동 탐지(`libnetfilter_queue.so.1` 등),
파일 목록, 임시 루트 설치, **상대 링크가 그 루트 안에서 정상 해석**, 패키지된 바이너리 실행.
`.deb`도 상대 링크로 재검증(11항목) 완료.

---

### 4.11 히스토리 재작성 — 완료 (2026-08-28)

`external/iup*`를 전체 히스토리에서 제거했습니다. 사용자가 "원 저장소와 병합할 일이 없다,
과거는 다 지워도 된다"고 확인해 진행했습니다.

**왜 워킹 트리 삭제만으로는 부족했나**: 4.9에서 파일을 지워도 git 객체는 히스토리에 남아
클론 용량이 그대로였습니다. 조사해 보니 히스토리에는 IUP가 **3.8 / 3.16 / 3.27 / 3.30
네 버전, 18개 디렉토리 변종, 비압축 192MB**로 쌓여 있었습니다. 나머지는 전부 소형입니다
(src 2MB, WinDivert 변종 각 0.4MB 미만).

**작업 내용**

- 먼저 전체 ref를 담은 백업 번들 생성 (`clumsy-backup-20260828-112521/clumsy-full.bundle`, 59MB)
- `git filter-repo --path-regex '^external/iup' --invert-paths`
  (glob이 아닌 앵커된 정규식 — 무엇이 매치되는지 모호하지 않게)
- `jlennox-master`(2015), `update-iup`(2015) 원격 브랜치 삭제 — IUP 객체를 붙들고 있어
  이걸 남기면 원격 용량이 줄지 않습니다. `update-iup` tip은 구 태그 `0.3rc2`와 동일 커밋이라
  태그 쪽에 보존됩니다.
- **`gh-pages`는 이 시점에 남겨뒀다가 4.13에서 삭제했습니다** — 당시 "서비스 중인 웹사이트"로
  판단했으나 확인해 보니 사실이 아니었습니다. 아래 4.13 참고.

**검증**

| 항목 | 결과 |
|------|------|
| HEAD 트리 해시 | `2bc7594...` — 재작성 전과 **완전히 동일** (워킹 트리 무변경) |
| 히스토리 내 IUP 객체 | 0개 |
| 커밋 / 태그 | 122개 / 6개 모두 보존 |
| `.git` 크기 | 58MB → **1.8MB** |
| 신규 클론 검증 | `.git` 1.9MB, 트리 해시 일치, IUP 0개 |
| 빌드 | Windows Debug/Release 경고 0, 리눅스 make 경고 0, 계약 테스트 16항목, `.deb` 정상 |

**영향 (사전 고지 후 승인받은 사항)**

- 커밋 해시 122개 전부 변경 → 기존 클론은 재클론 필요
- 태그 `0.1`~`0.3rc3`은 유지되지만 IUP가 빠져 **해당 릴리스를 태그에서 빌드할 수 없음**
- upstream(jagt/clumsy)과 히스토리가 갈라져 이후 병합 불가

되돌리려면 백업 번들에서 복원할 수 있습니다:
`git clone /path/to/clumsy-full.bundle restored`

---

### 4.12 Windows 실캡처 회귀 — 완료 (2026-08-28)

세션 내내 유일하게 미검증으로 남아 있던 항목입니다. `Start-Process -Verb RunAs`로 UAC
프롬프트를 띄우고 사용자가 승인하면, 승격된 clumsy를 REST API로 원격 제어해 자동 검증할
수 있었습니다. UAC 클릭 1회, 소요 약 1분.

하네스는 `tests/windows/capture_test.ps1`로 리포지토리에 포함했습니다 (**23항목 전부 통과**).

| 검증 | 결과 |
|------|------|
| WinDivert 드라이버 오픈 | 성공, `capturing:true` |
| 무손상 통과 | 20/20, 페이로드 동일, `captured=20 sent=20` (정확히 일치) |
| 기준 지연 | 1ms |
| drop 100% | 20 → **0** |
| **lag 400ms** | 실측 **407ms**, 버스트 20/20 무손실 |
| tamper | 페이로드 변조 + **20/20 도착** (체크섬 재계산이 정확하다는 증거) |
| duplicate ×3 | 10 → **정확히 30** |
| bandwidth 1KB/s | 120 → 48 전달, **72개 큐 대기** |
| pcap | magic `0xa1b2c3d4`, snaplen 65535, linktype 101, 실패킷 기록 |
| 정상 종료 | 캡처 중지 → idle → 프로세스 종료 → 웹서버 동반 종료 |

**하네스 작성 중 겪은 함정 2가지** (README에 기록):

1. 수신자를 `Start-Job`으로 띄웠더니 바인딩 전에 송신이 시작돼 아무것도 측정되지 않았습니다.
   유일하게 패킷을 받은 게 lag 케이스였는데, clumsy의 400ms 지연 덕에 잡이 준비될 시간이
   생겼던 것뿐입니다. 수신 소켓을 메인 런스페이스에서 미리 바인딩해 해결했습니다.
2. `Start-Process -Verb RunAs`는 출력 리다이렉션을 못 해 `.bat` 래퍼를 썼는데,
   스크립트가 만든 `.bat`이 실행 전에 사라졌습니다(보안 정책 추정). 로그가 필요했던 정보는
   전부 REST API로 얻을 수 있어 래퍼를 제거했습니다.

**bandwidth 관련 확인**: 초기에 "스로틀링 없음"으로 나왔으나 결함이 아니었습니다.
토큰 버킷이 `max(65535, limit*1024*2)`로 가득 찬 상태에서 시작해 작은 버스트는 설계상
통과합니다. 164KB를 밀어 넣으니 48/120 전달 + 72개 대기로 정확히 동작했습니다
(48×1400B ≈ 64KB로 버킷 초기값과 일치). 사용자가 오해하기 쉬운 지점이라 manual.md에
초기 버스트 허용량을 명시했습니다.

---

### 4.13 `gh-pages` 삭제 — 완료 (2026-08-28)

4.11에서 "서비스 중인 프로젝트 웹사이트"라는 이유로 남겨뒀던 브랜치입니다.
사용자가 "내가 만든 게 아니라 원본쪽 페이지 아니냐"고 지적해 확인해 보니 **두 가지 모두
사실과 달랐습니다.**

| 확인 항목 | 결과 |
|-----------|------|
| 작성자 | jagt 23커밋, Chen Tao 6커밋(동일인), Ng Yik Phang 2커밋 — **이 리포지토리 소유자 커밋 0개** |
| 기간 | 2013-10-13 ~ 2021-05-10, 전부 포크 이전 |
| 내용 | `index.html`이 소스로 **`github.com/jagt/clumsy`를 링크**, 0.2/0.3 다운로드 안내, 제거된 GUI 스크린샷 |
| GitHub Pages 활성 여부 | `gh api repos/.../pages` → **404**, `jacking75.github.io/clumsy` → **HTTP 404** |

즉 포크할 때 딸려온 upstream 웹사이트이며, **아무 데도 노출되지 않는 휴면 데이터**였습니다.
포크는 Pages가 자동으로 켜지지 않는다는 점을 4.11 시점에 확인하지 않은 것이 원인입니다.

켜더라도 upstream을 소스로 안내하고 0.4에는 존재하지 않는 GUI를 보여주므로 잘못된 내용입니다.
삭제 전 백업 번들에 `refs/remotes/origin/gh-pages 28912f7`이 보존되어 있음을 확인했습니다.

**결과**: 원격에 남은 브랜치는 `master` 하나입니다.
