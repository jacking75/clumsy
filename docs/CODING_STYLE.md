...
> 로드맵은 [docs/ROADMAP-v2.md](ROADMAP-v2.md), 아키텍처 개요는 [CLAUDE.md](../CLAUDE.md)를 참고하세요.
...
- 외부 라이브러리 의존 — 의존성은 WinDivert 하나로 묶어 둡니다. HTTP 서버·JSON 파서·pcap
  라이터·웹 UI는 필요한 기능만 직접 구현하는 것이 벤더링보다 위험이 낮다고 판단한 범위입니다.
